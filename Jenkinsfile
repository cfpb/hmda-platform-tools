        ...
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'dockerhub',
            ...
            withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'hmda-platform-jenkins-service',
              ...
              withCredentials([string(credentialsId: 'internal-docker-registry', variable: 'DOCKER_REGISTRY_URL')]){
                sh "docker build --rm -t=${env.DOCKER_HUB_USER}/hmda-platform-tools ."
                if (gitTagged || gitBranch == "master" || isDeployPR) {
                  //Push to Dockerhub
                  sh """
                    docker tag ${env.DOCKER_HUB_USER}/hmda-platform-tools ${env.DOCKER_HUB_USER}/hmda-platform-tools:${env.DOCKER_TAG}
                    docker login -u ${env.DOCKER_HUB_USER} -p ${env.DOCKER_HUB_PASSWORD} 
                    docker push ${env.DOCKER_HUB_USER}/hmda-platform-tools:${env.DOCKER_TAG}
                  """

                  //Push to Internal Docker Repo
                  sh """
                    docker tag ${env.DOCKER_HUB_USER}/hmda-platform-tools:${env.DOCKER_TAG} ${DOCKER_REGISTRY_URL}/${env.DOCKER_HUB_USER}/hmda-platform-tools:${env.DOCKER_TAG}
                    docker login ${DOCKER_REGISTRY_URL} -u ${env.DTR_USER} -p ${env.DTR_PASSWORD} 
                    docker push ${DOCKER_REGISTRY_URL}/${env.DOCKER_HUB_USER}/hmda-platform-tools:${env.DOCKER_TAG}
